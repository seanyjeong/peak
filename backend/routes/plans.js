...
router.get('/', verifyToken, async (req, res) => {
    try {
        ...
        // 해당 날짜에 배정된 학생이 있는 시간대 확인 - 해당 학원만
        const [assignedSlots] = await db.query(`
            SELECT DISTINCT time_slot FROM daily_assignments WHERE date = ? AND academy_id = ?
        `, [targetDate, academyId]);
        ...
        // 해당 날짜의 수업 계획 조회 - 해당 학원만
        let planQuery = `SELECT * FROM daily_plans WHERE academy_id = ? AND date = ?`;
...
router.post('/', verifyToken, async (req, res) => {
    try {
        ...
        // 같은 날짜, 시간대, 강사의 계획이 이미 있는지 확인 - 해당 학원만
        const [existing] = await db.query(
            'SELECT id FROM daily_plans WHERE academy_id = ? AND date = ? AND time_slot = ? AND instructor_id = ?',
            [academyId, date, time_slot, instructor_id]
        );
