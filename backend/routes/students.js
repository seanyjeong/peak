...
/**
 * GET /peak/students/:id/export-pdf - 학생 성적표 PDF (Playwright + Chart.js)
 */
router.get('/:id/export-pdf', verifyToken, async (req, res) => {
    try {
        ...
        function calcScore(value, typeId, gender) {
            const table = scoreTablesByType[typeId];
            ...
            for (const range of ranges) {
                ...
                if (value >= min && value <= max) { found = range.score; break; }
            }
            if (found === null) {
                if (isLowerBetter) {
                    for (const range of ranges) {
                        const max = gender === 'M' ? range.male_max : range.female_max;
                        if (value <= max) { found = range.score; break; }
                    }
                } else {
                    for (const range of ranges) {
                        const min = gender === 'M' ? range.male_min : range.female_min;
                        if (value >= min) { found = range.score; break; }
                    }
                }
                if (found === null) found = ranges[ranges.length - 1]?.score || 50;
            }
            return found;
        }
        ...
        recordTypes.forEach(rt => {
            ...
            bests[rt.id] = { value: bestValue, date: bestRecord?.measured_at };
            latests[rt.id] = { value: parseFloat(typeRecords[0].value), date: typeRecords[0].measured_at };
            const score = calcScore(parseFloat(typeRecords[0].value), rt.id, student.gender);
            if (score !== null) scores[rt.id] = score;
            ...
            if (typeRecords.length >= 5) {
                ...
                let imp = 0, dec = 0;
                for (let i = 0; i < 4; i++) {
                    if (isLowerBetter) { if (vals[i] < vals[i+1]) imp++; else if (vals[i] > vals[i+1]) dec++; }
                    else { if (vals[i] > vals[i+1]) imp++; else if (vals[i] < vals[i+1]) dec++; }
                }
                if (isLowerBetter) { if (vals[0] < vals[4]) imp += 2; else if (vals[0] > vals[4]) dec += 2; }
                else { if (vals[0] > vals[4]) imp += 2; else if (vals[0] < vals[4]) dec += 2; }
                trends[rt.id] = imp > dec ? 'up' : dec > imp ? 'down' : 'stable';
            } else {
                trends[rt.id] = 'need_more';
            }
        });
        ...
        const totalScore = scoreValues.reduce((a, b) => a + b, 0);
        const maxPossibleScore = scoreValues.length * 100;
        const percentage = maxPossibleScore > 0 ? Math.round((totalScore / maxPossibleScore) * 1000) / 10 : 0;
        let grade = 'F';
        if (percentage >= 90) grade = 'A';
        else if (percentage >= 80) grade = 'B';
        else if (percentage >= 70) grade = 'C';
        ...
        const upC = trendVals.filter(t => t === 'up').length;
        const downC = trendVals.filter(t => t === 'down').length;
        const overallTrend = upC > downC ? 'up' : downC > upC ? 'down' : 'stable';

        // 7. Academy score averages (same gender)
        const academyScoreAverages = {};
        try {
            const [allLatest] = await db.query(`
                SELECT sr.record_type_id, sr.value
                FROM student_records sr
                INNER JOIN (
                    SELECT student_id, record_type_id, MAX(measured_at) as max_date
                    FROM student_records WHERE academy_id = ?
                ...
                JOIN students s ON sr.student_id = s.id
                JOIN paca.students ps ON s.paca_student_id = ps.id AND ps.academy_id = ?
                WHERE sr.academy_id = ? AND ps.status = 'active' AND ps.gender = ?
            `, [academyId, academyId, academyId, student.gender]);

            const byType = {};
            allLatest.forEach(r => {
                const score = calcScore(parseFloat(r.value), r.record_type_id, student.gender);
                if (score !== null) {
                    if (!byType[r.record_type_id]) byType[r.record_type_id] = [];
                    byType[r.record_type_id].push(score);
                }
            });
            Object.entries(byType).forEach(([typeId, arr]) => {
                academyScoreAverages[typeId] = Math.round((arr.reduce((a, b) => a + b, 0) / arr.length) * 10) / 10;
            });
        } catch (e) {
            console.error('Academy averages error (non-fatal):', e.message);
        }
        ...
        const recordHistory = Object.keys(recordsByDate).sort().reverse()
            .slice(0, 10)
            .map(date => ({ measured_at: date, records: recordsByDate[date] }));

        // 9. Build input data for Python script
        const inputData = JSON.stringify({
            academyName,
            student: { name: student.name, gender: student.gender, school: student.school || '', grade: student.grade || '' },
            stats: {
                totalScore, maxPossibleScore, percentage, grade, overallTrend,
                recordCount: allRecords.length,
                typesWithRecords: Object.keys(latests).length,
                scores: Object.fromEntries(Object.entries(scores).map(([k, v]) => [String(k), v])),
                latests: Object.fromEntries(Object.entries(latests).map(([k, v]) => [String(k), v])),
                bests: Object.fromEntries(Object.entries(bests).map(([k, v]) => [String(k), v])),
                trends: Object.fromEntries(Object.entries(trends).map(([k, v]) => [String(k), v]))
            },
            recordTypes: recordTypes.map(rt => ({
                id: rt.id, name: rt.name, short_name: rt.short_name, unit: rt.unit, direction: rt.direction
            })),
            records: recordHistory,
            academyScoreAverages: Object.fromEntries(
                Object.entries(academyScoreAverages).map(([k, v]) => [String(k), v])
            )
        });

        // 10. Spawn Python Playwright PDF generator
        const tmpOutput = path.join(os.tmpdir(), `peak_pdf_${studentId}_${Date.now()}.pdf`);
        const scriptPath = path.join(__dirname, '../scripts/generate_pdf.py');

        await new Promise((resolve, reject) => {
            const child = spawn('python3', [scriptPath, '--output', tmpOutput], {
                timeout: 30000,
            ...
            child.on('close', (code) => {
                if (code !== 0) reject(new Error(`PDF script exit ${code}: ${stderr}`));
                else resolve();
            });
        ...
        // 11. Send PDF
        const fileName = encodeURIComponent(`${student.name}_실기기록_${new Date().toISOString().split('T')[0]}.pdf`);
